...
> **⚠️ 已废弃（2026-08）**：股价历史现由批处理**云端自动抓取**（腾讯行情 `web.ifzq.gtimg.cn`
> 前复权日线，A股+港股通用，无需 WAF 规避），无需再手动下载 kline 文件。下方手动流程
> 仅作**应急后备**（腾讯源长期不可用时）保留；正常情况下只需点「更新」。若某次抓取失败，
> 该股详情页顶部会提示，股价指标显示为上次数据。

财务报表和分红由季度批处理**云端实时抓取**，无需手动维护。
只有 **kline 股价历史**曾需要手动更新，因为 EastMoney 的 `push2his` 接口
