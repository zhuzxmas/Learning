...
每 ~14 天自动读取**期间发表的生活博客与评论、贴吧新主题/新回复、旅行记录、AI 对话和日历记录**，
交给 **DeepSeek** 汇总成一份中文 Markdown，写回 OneDrive **博客文件夹的
`summaries/`** 子文件夹。评论按发表时间独立纳入，不会重复原文章；旧博客在本期修改不会纳入；贴吧按活动时间筛选，旅行按实际旅行日期筛选。

AI 对话按每条消息的发表时间筛选，仅纳入窗口内新增且带时间戳的消息，并保留对话标题；
旧版无逐条时间戳的历史消息不会再次纳入。
