...
const noDa = periods.map((row) => Object.assign({}, row, { depreciation_amortization: null }));
const noDaResult = calculate(noDa);
assert.ok(noDaResult.epv);
assert.equal(noDaResult.epv.normalized_depreciation_amortization, 0);
assert.equal(noDaResult.epv.maintenance_capex, 0);
assert.equal(noDaResult.epv.per_share, 4.2);
